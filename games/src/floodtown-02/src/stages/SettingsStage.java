...
public class SettingsStage extends Stage {
    ...
    public SettingsStage(StageArguments stageArguments) {
        ...
        backToMenuButton.setClickAction(() -> ((FloodtownStageManager) stageArguments.getStageManager()).setMainMenuStage());
        ...
        addSampleImageForRenderingPreview();
    ...
    private void addSampleImageForRenderingPreview() {
        add(new GameObject() {
            @Override
            public void render(@NotNull Graphics2D graphics2D) {
                graphics2D.setColor(new Color(210, 210, 210));
                graphics2D.drawString("Rendering with current settings:", 700, 160);
                AffineTransform affineTransform = new AffineTransform();
                affineTransform.translate(700, 170);
                affineTransform.scale(0.4, 0.4);
                graphics2D.drawImage(SpriteLoader.load(Sprites.coalPlant), affineTransform, null);
                affineTransform.translate(90, 50);
                graphics2D.drawImage(SpriteLoader.load(Sprites.farm), affineTransform, null);
            }
        });
    }
